...
(function ($, fluid) {
    ...
    var formatIDNumber = function (id_num) {
        // split id_num into parts
        // "OL2019.1.2" => ["OL2019", "1", "2"]
        var id_num_parts = id_num.split(".");

        // check if our array only has one element
        if (id_num_parts.length === 1) {
            return id_num;
        }

        var new_id_num = [];

        // split off the prefix of the num_part and push it into the new_id_num array
        new_id_num.push(id_num_parts.shift());

        var total_parts = id_num_parts.length;

        $.each(id_num_parts, function(index, value){
            // check if the element is an integer
            if (/^\d+$/.test(value)){
                // if this is the last index then use 4 character padding
                // but only if there is two or more integer parts
                if (total_parts >= 2 && index === total_parts - 1) {
                    new_id_num.push(pad(value, 4));
                } else { // otherwise, use 3 character padding
                    new_id_num.push(pad(value, 3));
                }
            }
        });

        // return array as string
        // ["OL2019","001", "00002"] => "OL2019.001.0002"
        return new_id_num.join(".");
    }
    ...
    var populateInputField = function (that) {
        return function (data, status) {
            ...
            // apply formatter to data.next
            numField.val(formatIDNumber(data.next));
            numField.change();
            numField.focus();
        };
    };
