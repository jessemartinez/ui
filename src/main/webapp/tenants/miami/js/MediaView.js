    ...
    cspace.mediaView.preInit = function (that) {
        that.prepareModelForRender = function () {
            ...
            fluid.each(that.model.relatedMedia, function (thisMedia) {
                // Check if this media record is primary
                if (thisMedia.primaryMedia.length > 0) {
                    // Add this object to the primaryRelatedMedia array
                    that.model.primaryRelatedMedia.unshift(thisMedia);
                    // In case there is more than one related media records labeled as primary,
                    // pick out the more recently updated record as the first of the media array
                    media.unshift(that.model.primaryRelatedMedia[0]);
                } else {
