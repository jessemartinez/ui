...
(function ($, fluid) {
        ...
        components: {
            ...
            popup: {
                ...
                options: {
                    ...
                    termStatusTriggers: "{autocomplete}.options.termStatusTriggers"
                }
            },
        ...
        cspace.util.preInitMergeListeners(that.options, {
            prepareModelForRender: function (model, applier, that) {
                var matches = [],
                    ...
                    termStatus = that.options.elPaths.termStatus,
                ...
                fluid.each(fluid.get(model, matchesPath), function (match) {
                    ...
                    matches = matches.concat(fluid.transform(displayNameList, function (thisDisplayName, index) {
                        ...
                        elem[termStatus] = match.termStatus;
                        elem[rowDisabled] = (!elem[preferred] && (disabled === true));
                        elem[namespace] = match.namespace;

                        // check term list values against trigger list and disable if matched
                        fluid.each(that.options.termStatusTriggers, function(termStatusTriggerItem) {
                            if (termStatusTriggerItem.type === match.type) {
                                fluid.each(termStatusTriggerItem.terms, function(term) {
                                    if (term === match.termStatus) {
                                        elem[displayName] = thisDisplayName + " (" + term + ")";
                                        elem[rowDisabled] = true;
                                        return false;
                                    }
                                });
                                return false;
    ...
    fluid.defaults("cspace.autocomplete.popup", {
        ...
        styles: {
            ...
            rejectedTerm: "cs-autocomplete-rejectedTerm"
        },
